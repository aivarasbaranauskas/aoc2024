...
let read_input () = List.map (fun x -> (int_of_char x) - (int_of_char '0')) @@ Shared.F.read_char_list file
...
let expand disk_map = 
  ...
  let rec do_expand i i2 dm = 
    ...
    | [] -> ()
    | [x] -> do_fill i x i2
    | x1::x2::t -> do_fill i x1 i2; do_expand (i+x1+x2) (i2+1) t
  in
  do_expand 0 0 disk_map;
  arr

let calculate_hash disk_map = Shared.L.sum
  @@ Array.to_list 
...
let expand_2 disk_map = 
  Array.of_list @@List.flatten @@ List.map (fun (f, s, id) -> List.init (f+s) (fun i -> if i<f then id else -1)) (Array.to_list disk_map)
...
let part2 () = 
  ...
  let rec rearrange i1 =
    ...
    in
    if i1 < 0 then () else (if rearrange_inner 0 then rearrange i1 else rearrange(i1-1) )
  in
