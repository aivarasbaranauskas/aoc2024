...
let read_input () = 
  let rec parse lines acc =
    match lines with
    | ""::tail -> acc, (List.map (fun line -> List.map int_of_string @@ String.split_on_char ',' line) tail)
    | s::tail -> let spl = String.split_on_char '|' s in parse tail ((int_of_string @@ List.nth spl 0 , int_of_string @@ List.nth spl 1)::acc)
    | _ -> raise (Failure "bad")
  in
...
let check_update rules update = 
  let rec check u = match u with
    | [] -> true
    | ui::t -> 
      let fails_any_rule = List.exists (fun (a, b) -> ui = b && (List.mem a t ) ) rules in
      if fails_any_rule 
        then false
      else  check t
  in
...
let fix_update rules update =
  let rec fix h u = match u with
    | [] -> List.rev h
    | ui::t -> 
      let failing_rule = List.find_opt (fun (a, b) -> ui = b && (List.mem a t ) ) rules in
...
let part2 () =
  let rules, updates = read_input() in
  let filtered_bad_updates = List.filter (fun u -> not @@ check_update rules u) updates in
